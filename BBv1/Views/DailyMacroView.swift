...
//        self.kcal = healthKitManager.caloriesToday
    ...
    var body: some View {
        VStack{
        Chart{
            ForEach(Array(zip(self.macroVals, self.macroNames)), id: \.0) { item in
                SectorMark(angle: .value("g", item.0), innerRadius: .ratio(0.6),
                           angularInset: 8).foregroundStyle(by: .value("Macro", item.1))
            ...
            PieChartRows(colors: self.colors, names: self.macroNames, values: self.macroVals.map { self.formatter(Double($0)) }, percents: self.macroVals.map { String(format: "", $0 * 100 / self.macroVals.reduce(0, +)) })
        }
    }
